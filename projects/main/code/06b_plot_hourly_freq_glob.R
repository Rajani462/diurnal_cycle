...
ggplot(spat_mean_dt) + 
  ...
  scale_fill_binned(type = "viridis", option = "B", direction = -1, 
                    breaks = c(0.3, 0.6, 0.9, 1.2, 1.5, 2, 3, 5), show.limits = TRUE) + 
...
ggsave("./projects/main/results/06c_spat_freq_threshold_0.1.png", width = 9.5, height = 5.3, 
       units = "in", dpi = 600)
...
mean_freq_list <- lapply(freq_list, function(dt) {
  dt[, .(mean_value = mean(prec_freq, na.rm = TRUE)), by = .(hour(time_lst), name)]
})
...
ggsave("./projects/main/results/06c_24hlineplot_freq_threshold_0.1.png",
...
ggsave("./projects/main/results/06c_faceted_24hlineplot_freq_threshold_0.1.png",
       width = 8.9, height = 5.6, units = "in", dpi = 600)
...
system.time(peak_hour_list <- parLapplyLB(cl, freq_list, function(dt) {
  # Modify the code to use the data.table "freq_freq"
  peak_hour <- dt[, .SD[which.max(prec_freq)], keyby = .(lat, lon, name)]
...
ggsave("./projects/main/results/06c_plot_spat_peak_hour_freq_threshold_0.1.png", width = 10.5, height = 6.9, 
       units = "in", dpi = 600)
...
freq_land_list <- readRDS("./projects/main/data/hourly/hourly_freq_all_datasets_LST_land_2001_20.rds")
freq_ocn_list <- readRDS("./projects/main/data/hourly/hourly_freq_all_datasets_LST_ocn_2001_20.rds")

freq_list <- c(freq_land_list, freq_ocn_list)
...
levels(mean_freq_24h$name)
levels(mean_freq_24h$name) <- c("IMERG", "GSMaP", "CMORPH", "PERSIANN", "ERA5")
levels(mean_freq_24h$type) <- c("Land", "Ocean")
...
ggsave("./projects/main/results/06b_24hlineplot_freq_land_ocn_threshold_0.1.png",
       width = 8.9, height = 5.6, units = "in", dpi = 600)
