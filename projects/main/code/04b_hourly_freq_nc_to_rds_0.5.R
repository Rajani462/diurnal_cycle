...
for (dataset in datasets) {
  for (threshold in thresholds) {
    file_name <- paste0("hourly_freq_", dataset, "_", threshold)
...
process_dataset <- function(file_path) {
  ...
  col_name <- ifelse(grepl("_0\\.2$", dataset_name), "prec_freq_0.2", "prec_freq_0.5")
...
new_col_names <- c("x", "y", "date", "prec_freq_0.2", "prec_freq_0.5", "name")
...
dat_lst_list <- lapply(merged_list, function(dt) {
  ...
  dt <- dt[, .(lat = y, lon = x, time_utc = as_hms(date), prec_freq_0.2, prec_freq_0.5, name, location)]
...
saveRDS(dat_lst_list, "./projects/main/data/hourly_freq_thres_0.2_0.5_all_datasets_LST_glob_2001_20.rds")
