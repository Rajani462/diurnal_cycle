...
data_list1 <- readRDS("./projects/main/data/hourly_mean_thres_0.2_0.5_all_datasets_LST_glob_2001_20.rds")
...
for (i in seq_along(rast_robin_df)) {
  setnames(rast_robin_df[[i]], 1, "prec_mean")
}

rast_robin_dt <- rbindlist(rast_robin_df)

#to_plot <- melt(rast_robin_dt, c("x", "y", "name", " threshold"), variable.name = "threshold")
to_plot <- rast_robin_dt[, .(x, y, name = factor(name), threshold = factor(threshold), value = round(prec_mean, 2))]
