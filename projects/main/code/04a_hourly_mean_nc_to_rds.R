...
merged_output_list <- lapply(output_list, function(dataset) merge(dataset, mask_table, by = c("x", "y")))
...
for (dataset in datasets) {
  for (season in seasons) {
    ...
    if (dataset == "persiann") {
      file_name <- paste0(file_name, "_jja") # Only "jja" for persiann
    }
    file_path <- paste0("~/shared/data_downloads/input_data/seasonal/hourly_character/", file_name, ".nc")
    file_paths <- c(file_paths, file_path)
  }
...
process_dataset <- function(file_path) {
  ...
  # Apply the date conversion for "persiann" dataset
  if (dataset_name == "persiann") {
    pers_time <- getZ(dataset)
    posixct_time <- as.POSIXct(pers_time * 3600, origin = "2001-01-01 00:00:00")
    dataset_dt[, date := posixct_time]
  }
  
  dataset_dt[, `:=`(name = factor(dataset_name), season = season)]
