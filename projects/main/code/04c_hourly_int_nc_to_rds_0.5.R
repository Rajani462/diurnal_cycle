...
dest  <- "~/shared/data_downloads/input_data/seasonal/hourly_character/"

# test with 1 month (31 files) of datasets
#file_name <- list.files(dest, pattern = ".*3IMERG.201901.*\\.nc4$", full.names = TRUE) %>% as.list()
file_name <- list.files(dest, pattern = "^hourly_freq_.*_0\\.5\\.nc$", full.names = TRUE) %>% as.list()



for (dataset in datasets) {
  for (season in seasons) {
    file_name <- paste0("hourly_freq_", dataset, "_tp_mm_60ns_", ifelse(dataset == "gsmap", "2015_20", "2001_20"), "_025_hourly_", "_0.5")
