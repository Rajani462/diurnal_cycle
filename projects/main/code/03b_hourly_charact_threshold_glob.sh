...
for input_file in ~/shared/data_downloads/input_data/*.nc; do
    ...
    # Run the cdo command
    cdo -b 32 -P 50 -dhourmean -expr,'count_value_above_0_5 = (precip >= 0.5 ? precip : 0)' "$input_file" "$output_file"
done
...
for input_file in ~/shared/data_downloads/input_data/*.nc; do
    ...
    # Run the cdo command
    cdo -P 43 -div -dhoursum -mul "$input_file" -expr,'count_value_above_0_5 = (precip >= 0.5 ? 1 : 0)' "$input_file" -dhoursum -expr,'count_value_above_0_5 = (precip >= 0.5 ? 1 : 0)' "$input_file" "$output_file"
done
