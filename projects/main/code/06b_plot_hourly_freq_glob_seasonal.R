...
spat_freq_dt <- data_dt[, .(mean_value = round(mean(prec_freq, na.rm = TRUE), 2)), by = .(lat, lon, name, season)]

summary(spat_freq_dt)
...
ggplot(spat_freq_dt) + 
  ...
  coord_cartesian(xlim = c(min(spat_freq_dt$lon), max(spat_freq_dt$lon)), 
                  ylim = c(min(spat_freq_dt$lat), max(spat_freq_dt$lat))) + 
...
ggsave("./projects/main/results/06b_24hlineplot_freq_landocn_seasonal.png",
       width = 8.9, height = 5.6, units = "in", dpi = 600)
