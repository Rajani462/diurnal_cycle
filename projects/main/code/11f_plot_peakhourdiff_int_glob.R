...
ggsave("./projects/main/results/11f_plot_hist_peak_hour_diff_int.png", width = 10.5, height = 5.1, 
       units = "in", dpi = 600)
...
summary(result)
setnames(result, c("name.x"), c("name"))

peak_hour_list <- split(result, by = "name")

peak_hour_list <- lapply(peak_hour_list, function(df) df[, c("lon", "lat", "time_diff_lst", "name")])
...
peak_hour_list$gsmap <- peak_hour_list$gsmap[name == "imerg", time_diff_lst := NA]
peak_hour_list$gsmap[is.na(peak_hour_list$gsmap$time_diff_lsts), "name"] <- "gsmap"
...
peak_hour_list$persiann <- peak_hour_list$persiann[name == "imerg", time_diff_lst := NA]
peak_hour_list$persiann[is.na(peak_hour_list$persiann$time_diff_lsts), "name"] <- "persiann"

peak_hour_list$cmorph <- rbind(peak_hour_list$cmorph, imerg)
peak_hour_list$cmorph <- unique(peak_hour_list$cmorph, by = c("lat", "lon"))
peak_hour_list$cmorph <- peak_hour_list$cmorph[name == "imerg", time_diff_lst := NA]
peak_hour_list$cmorph[is.na(peak_hour_list$cmorph$time_diff_lsts), "name"] <- "cmorph"
...
# Assuming peak_hour_dt is your data frame
peak_hour_dt$name <- factor(peak_hour_dt$name, levels = c("imerg", "gsmap", "cmorph", "persiann", "era5"))
levels(peak_hour_dt$name)
...
ggsave("./projects/main/results/11f_plot_spat_peak_hour_diff_int.png", width = 7.1, height = 5.1, 
       units = "in", dpi = 600)
...
ggsave("./projects/main/results/11f_plot_spat_peak_hour_diff_int2.png", width = 7.1, height = 5.1, 
       units = "in", dpi = 600)
