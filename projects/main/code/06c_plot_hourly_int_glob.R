...
spat_mean_dt <- data_dt[, .(mean_value = round(mean(prec_int, na.rm = TRUE), 2)), by = .(lat, lon, name)]

summary(spat_mean_dt)
spat_mean_dt[complete.cases(spat_mean_dt), ]

ggplot(spat_mean_dt) + 
  geom_raster(aes(lon, lat, fill = mean_value)) +
  scale_fill_binned(type = "viridis", option = "B", direction = -1, 
                    breaks = c(0.2, 0.4, 0.6, 0.8, 1, 1.5, 2, 3, 5, 10), show.limits = TRUE) + 
...
ggsave("./projects/main/results/06a_spat_int.png", width = 9.5, height = 5.3, 
       units = "in", dpi = 600)
...
data_dt[complete.cases(data_dt), ]
...
ggsave("./projects/main/results/06a_24hlineplot_int_glob.png",
...
ggsave("./projects/main/results/06a_24hlineplot_int_landocn.png",
       width = 8.9, height = 5.6, units = "in", dpi = 600)
...
ggsave("./projects/main/results/06a_24hlineplot_int_landocn_seas.png",
       width = 8.9, height = 5.6, units = "in", dpi = 600)


### Estimate the peak hour of data.tables -------------------------------------------

system.time(peak_hour_dt <- data_dt[, .SD[which.max(prec_int)], by = .(lat, lon, name)])
# user  system elapsed 
# 488.337   2.325 362.277 
saveRDS(peak_hour_dt, "./projects/main/data/int_peak_hour_dt_2001_20.RDS")

peak_hour_dt <- readRDS("./projects/main/data/int_peak_hour_dt_2001_20.RDS")
...
ggsave("./projects/main/results/06a_plot_spat_peak_hour_int.png", width = 10.5, height = 6.9, 
       units = "in", dpi = 600)
...
ggsave("./projects/main/results/06a_plot_spat_peak_hour_int_2.png", width = 10.5, height = 6.9, 
       units = "in", dpi = 600)


### seasonal

system.time(peak_hour_seas_dt <- data_dt[, .SD[which.max(prec_int)], by = .(lat, lon, name, season)])
# user  system elapsed 
# 865.650   2.789 729.145 

saveRDS(peak_hour_seas_dt, "./projects/main/data/int_peak_hour_seas_dt_2001_20.RDS")
#peak_hour_seas_dt <- readRDS("./projects/main/data/int_peak_hour_seas_dt_2001_20.RDS")
...
ggsave("./projects/main/results/06a_plot_spat_peak_hour_int_season.png", width = 10.5, height = 7.9, 
       units = "in", dpi = 600)
...
ggsave("./projects/main/results/06a_plot_spat_peak_hour_int_season_2.png", width = 10.5, height = 7.9, 
       units = "in", dpi = 600)
###############################################################