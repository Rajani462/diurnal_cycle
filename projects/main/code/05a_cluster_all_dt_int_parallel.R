...
cluster_data <- function(data_dt, k_value = 3, nstart_value = 50) {
...
process_datasets_parallel <- function(data_list, k_value = 3, nstart_value = 50) {
...
dat_list <- dat_list[c("era5")]
# Process all datasets in parallel
dat_list_cleaned <- lapply(dat_list, function(df) df[complete.cases(df), ])
processed_data_list <- process_datasets_parallel(dat_list_cleaned)
...
saveRDS(processed_data_list, "./projects/main/data/cluster_3_all_dat_int_parellel_setseed123.RDS")
