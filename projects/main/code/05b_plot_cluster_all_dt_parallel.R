...
# Define the conditions and update the cluster_name column
to_plot2[name %in% c("GSMaP", "CMORPH") & clusters == 1, cluster_name := "Afternoon peak"]
...
to_plot2[name %in% c("GSMaP", "CMORPH", "PERSIANN") & clusters == 3, cluster_name := "Early-morning peak"]
...
to_plot2[name == "PERSIANN" & clusters == 2, cluster_name := "Mid-day peak"]
to_plot2[name == "ERA5" & clusters == 1, cluster_name := "Mid-day peak"]
to_plot2[name == "GSMaP" & clusters == 4, cluster_name := "Mid-day peak"]
...
spat_plot <- ggplot() +
  ...
  scale_fill_manual(values = line_colors, breaks = c("Afternoon peak", "Early-morning peak", "Late-morning peak", 
                                                     "Mid-day peak", "Mid-night peak")) + 
  facet_wrap(~name, ncol = 3) + 
...
ggsave("./projects/main/results/cluster/05b_comb_all_data_cluster_4_spat_plot_named_robin.png", p, width = 10.5, height = 5.1, 
       units = "in", dpi = 600)
