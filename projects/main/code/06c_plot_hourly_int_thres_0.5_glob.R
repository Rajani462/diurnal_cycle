...
for (i in seq_along(rast_robin_df)) {
  setnames(rast_robin_df[[i]], 1, "prec_int")
}

rast_robin_dt <- rbindlist(rast_robin_df)

#to_plot <- melt(rast_robin_dt, c("x", "y", "name", " threshold"), variable.name = "threshold")
to_plot <- rast_robin_dt[, .(x, y, name = factor(name), threshold = factor(threshold), value = round(prec_int, 2))]
...
mean_24h_landocn <- data_dt[, .(prec_int_0.1 = mean(prec_int, na.rm = TRUE), 
                                prec_int_0.2 = mean(prec_int_0.2, na.rm = TRUE), 
