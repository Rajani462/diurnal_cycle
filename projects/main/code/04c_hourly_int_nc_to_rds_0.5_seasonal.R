...
for (dataset in datasets) {
  for (season in seasons) {
    file_name <- paste0("hourly_int_", dataset, "_", season)
...
dat_lst_list <- lapply(merged_list, function(dt) {
  ...
  dt <- dt[, .(lat = y, lon = x, time_utc = as_hms(date), prec_int, name, season, location)]
...
saveRDS(dat_lst_list, "./projects/main/data/hourly_int_all_datasets_LST_glob_2001_20_seasonal.rds")
...
for (dataset in datasets) {
  for (season in seasons) {
    file_name <- paste0("hourly_int_", dataset, "_", season, "_", "0.5")
    # if (dataset %in% c("imerg", "gsmap")) {
    #   file_name <- paste0(file_name, "_fliptrans")
    # }
    file_path <- paste0("~/shared/data_projects/diurnal_precip/processed/", file_name, ".nc")
...
extract_dataset_name <- function(file_path) {
  # Extract the part of the file name that comes after "hourly_mean_"
  start_index <- regexpr("hourly_mean_", file_path) + nchar("hourly_int_")
  sub_path <- substr(file_path, start_index, nchar(file_path))
...
for (dataset in datasets) {
  for (season in seasons) {
    for (threshold in thresholds) {
      file_name <- paste0("hourly_int_", dataset, "_", season, "_", threshold)
      # if (dataset %in% c("imerg", "gsmap")) {
      #   file_name <- paste0(file_name, "_fliptrans")
      # }
      file_path <- paste0("~/shared/data_projects/diurnal_precip/processed/", file_name, ".nc")
...
process_dataset <- function(file_path) {
  ...
  dataset_dt <- dataset_dt[, .(lat = y, lon = x, time_utc = as_hms(date), prec_int, name, season, threshold)]
...
saveRDS(merged_list, "./rajani/diurnal_cycle/projects/main/data/hourly_int_thres_0.5_all_datasets_LST_glob_2001_20_seasonal.rds") 

##################
merged_list <- readRDS("./rajani/diurnal_cycle/projects/main/data/hourly_int_thres_0.5_all_datasets_LST_glob_2001_20_seasonal.rds")
dat_list <- readRDS("./rajani/diurnal_cycle/projects/main/data/hourly_int_all_datasets_LST_glob_2001_20_seasonal.rds")
...
dat_list <- # Add the 'threshold' column directly to each dataset in the list
  dat_list <- lapply(dat_list, function(dataset) { 
    dataset <- dataset[, .(lon, lat, time_utc, prec_int, name, season, threshold = factor("0.1"), tmz_offset, time_lst, location)]
    return(dataset)
  })
...
saveRDS(comb_list, "./rajani/diurnal_cycle/projects/main/data/hourly_int_thres_0.1_0.5_all_datasets_LST_glob_2001_20_seasonal.rds")
