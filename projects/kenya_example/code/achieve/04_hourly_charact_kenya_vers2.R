...
imerg <- raster("~/rajani/diurnal_cycle/projects/kenya_example/data/mean_imerg_kenya_2001_20.nc")
...
#global
imerg_mean <- raster("~/shared/data_downloads/IMERGHHF_2015/imerg_60ns_2015_hourlymean.nc")
plot(imerg_mean)

imerg_mean_dt <- as.data.frame(imerg_mean, xy = TRUE) %>%
  as.data.table() %>%
  melt(., id.vars = c("x", "y"), variable.name = "date", value.name = "prec_mean") %>%
  `[`(, name := factor("imerg"))

summary(imerg_mean_dt)
imerg_mean_dt[prec_mean >= 0.97]

#corrected and verified the plot with panoply

#https://gis.stackexchange.com/questions/170137/converting-binary-trmm-to-geotiff-using-r/170173#170173

x <- t(imerg_mean)
plot(x)

a.flipy = flip(x, direction = 2) 
a.t.flipxy = flip(a.flipy, direction = 1)
plot(a.t.flipxy) #looks similar to panoply

imerg_mean_cor_dt <- as.data.frame(a.t.flipxy, xy = TRUE) %>%
  as.data.table() %>%
  melt(., id.vars = c("x", "y"), variable.name = "date", value.name = "prec_mean") %>%
  `[`(, name := factor("imerg"))

summary(imerg_mean_cor_dt)  
imerg_mean_cor_dt[prec_mean >= 0.97]

#Note: even though the plots looks difference due to order of x and y, the values remains similar in both case

# imerg_hour <- raster("~/rajani/diurnal_cycle/projects/kenya_example/data/hourly_charact/hourly_mean_imerg_kenya_2001_20.nc")
# era_hour <- raster("~/rajani/diurnal_cycle/projects/kenya_example/data/hourly_charact/hourly_mean_era5_kenya_2001_20.nc")
# cmorph_hour <- raster("~/rajani/diurnal_cycle/projects/kenya_example/data/hourly_charact/hourly_mean_cmorph_kenya_2001_20.nc")
# pers_hour <- raster("~/rajani/diurnal_cycle/projects/kenya_example/data/hourly_charact/hourly_mean_persiann_kenya_2001_20.nc")
# 
...
imerg_mean_dt <- as.data.frame(imerg, xy = TRUE) %>%
  as.data.table() %>%
  melt(., id.vars = c("x", "y"), variable.name = "date", value.name = "prec_mean") %>%
  `[`(, name := factor("imerg"))
...
# zvalues <- getZ(cmorph_hour)
# era_ocn <- setZ(era_ocn, zvalues, name='time')
# 
# getZ(pers_hou)
#convert the above data to data,table format and save

imerg_dt <- as.data.frame(imerg_hour, xy = TRUE) %>%
  as.data.table() %>%
  melt(., id.vars = c("x", "y"), variable.name = "date", value.name = "prec_mean") %>%
  `[`(, name := factor("imerg"))
...
gsmap_dt <- as.data.frame(gsmap_hour, xy = TRUE) %>%
  as.data.table() %>%
  melt(., id.vars = c("x", "y"), variable.name = "date", value.name = "prec_mean") %>%
  `[`(, name := factor("gsmap"))
