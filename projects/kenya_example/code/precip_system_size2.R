...
prec_system_list <- parLapply(cl, prec_system_list[1:10], function(x) {
...
#final_list <- lapply(1:691200, function(i) rbindlist(lapply(prec_system_list, function(df) df[i])))
clusterExport(cl, c("prec_system_list"))
system.time(final_list <-
              parLapply(cl, 1:691200, function(i) rbindlist(lapply(prec_system_list, function(df) df[i]))))
...
#result_list <- mclapply(prec_system_count_list, split_data_table, mc.cores = cores) #taking very long
prec_system_list <- parLapply(cl, prec_system_list, split_data_table) #taking too much of memory and crashing


result_list2 <- unlist(result_list, recursive = FALSE)
str(result_list2)

grouped_lists <- split(result_list2, names(result_list2))

final_list <- lapply(grouped_lists, rbindlist)
